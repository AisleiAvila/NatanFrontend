...
export class DashboardComponent implements OnInit {
  ...
  filterTodaySchedule(): void {
    ...
    this.todaySchedule = this.assignedRequests.filter(
      (request) =>
  ...
  calculateStats(): void {
    ...
    this.stats.completedToday = this.assignedRequests.filter(
      (r) =>
        r.status === "completed" &&
        new Date(r.updatedAt).toDateString() === new Date().toDateString()
    ).length;
    this.stats.pendingRequests = this.assignedRequests.filter(
      (r) => r.status === "pending"
    ).length;
  }
  ...
  toggleAvailability(): void {
    ...
    this.snackBar.open(message, this.translate("common.close"), {
      duration: 3000,
    });
  }
