...
function getQuoteCreationView(requestId = null) {
  const request = requestId
    ? natanApp.requests.find((r) => r.id === requestId)
    : null;
...
function addQuoteItem() {
  ...
  newItem.style.cssText =
    "display: grid; grid-template-columns: 2fr 1fr 1fr 1fr auto; gap: 15px; align-items: center; margin-bottom: 15px; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px;";
            ...
            <div style="background: white; border-radius: 15px; padding: 40px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px;">
                    <div>
                        ...
                        <p style="margin: 5px 0; color: #666;"><strong>Nome:</strong> ${
                          request.clientName
                        }</p>
                        <p style="margin: 5px 0; color: #666;"><strong>E-mail:</strong> ${
                          request.email
                        }</p>
                        <p style="margin: 5px 0; color: #666;"><strong>Telefone:</strong> ${
                          request.phone
                        }</p>
                        <p style="margin: 5px 0; color: #666;"><strong>Local:</strong> ${
                          request.location
                        }</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p style="margin: 5px 0; color: #666;"><strong>Data:</strong> ${new Date().toLocaleDateString(
                          "pt-PT"
                        )}</p>
                        <p style="margin: 5px 0; color: #666;"><strong>Válido até:</strong> ${new Date(
                          quoteData.validUntil
                        ).toLocaleDateString("pt-PT")}</p>
                        <p style="margin: 5px 0; color: #666;"><strong>Prazo:</strong> ${
                          quoteData.executionDays
                        } dias</p>
                        <p style="margin: 5px 0; color: #666;"><strong>Serviço:</strong> ${
                          request.serviceName
                        }</p>
                    </div>
                </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #dee2e6;">
                                ...
                                <td style="padding: 12px; text-align: right; color: #666;">€${item.unitPrice.toFixed(
                                  2
                                )}</td>
                                <td style="padding: 12px; text-align: right; color: #666; font-weight: 600;">€${item.total.toFixed(
                                  2
                                )}</td>
                            </tr>
                            `
                ...
                <div style="border: 2px solid #e0e0e0; border-radius: 10px; padding: 20px; margin-bottom: 30px;">
                    <div style="display: grid; gap: 10px;">
                        <div style="display: flex; justify-content: space-between;">
                            ...
                            <span style="font-weight: 600;">€${quoteData.subtotal.toFixed(
                              2
                            )}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
                            <span style="color: #666;">IVA (${
                              quoteData.ivaRate
                            }%):</span>
                            <span style="font-weight: 600;">€${quoteData.ivaAmount.toFixed(
                              2
                            )}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; border-top: 2px solid #ddd; padding-top: 10px; font-size: 1.2rem;">
                            ...
                            <span style="color: #1976d2; font-weight: 700;">€${quoteData.totalAmount.toFixed(
                              2
                            )}</span>
                        </div>
                    </div>
...
function generateUsersList() {
  ...
  return users
    .map(
      (user) => `
    <div class="user-item" data-role="${
      user.role
    }" style="border: 1px solid #e0e0e0; border-radius: 10px; padding: 20px; margin-bottom: 15px; display: grid; grid-template-columns: 1fr auto; gap: 20px; align-items: center;">
        <div>
            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 10px;">
                <div style="width: 50px; height: 50px; background: ${getRoleColor(
                  user.role
                ...
                }; color: white; padding: 4px 12px; border-radius: 15px; font-size: 12px; font-weight: 600;">
            ...
            <div style="display: flex; gap: 30px; color: #666; font-size: 14px;">
                ...
                <span><strong>Último acesso:</strong> ${new Date(
                  user.lastLogin
                ).toLocaleDateString("pt-PT")}</span>
                ${
                  user.role === "client"
                    ? `<span><strong>Solicitações:</strong> ${user.requests}</span>`
                    : ""
                }
                ${
                  user.role === "provider"
                    ? `<span><strong>Serviços:</strong> ${user.services}</span><span><strong>Avaliação:</strong> ${user.rating}★</span>`
                    : ""
                }
            </div>
        </div>
        ...
        <div style="display: flex; gap: 10px;">
            <button onclick="viewUser(${
              user.id
            })" style="background: #1976d2; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-size: 12px;">Ver</button>
            <button onclick="editUser(${
              user.id
            })" style="background: #ff9800; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-size: 12px;">Editar</button>
            <button onclick="toggleUserStatus(${user.id})" style="background: ${
        user.status === "active" ? "#f44336" : "#4caf50"
      }; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-size: 12px;">
...
function confirmQuote() {
  alert(
    "Orçamento confirmado e enviado por e-mail!\nO cliente receberá uma notificação."
  );
  goBack();
}
